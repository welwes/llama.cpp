...
struct llama_h1ec {
    ...
    // H1EC_MAX_BATCH (деф. 8): до какого размера пачки h1-сплит активен.
    // 16 пускает полные DFlash-блоки верификации через кэш (спекуляция);
    // ⚠ отрицательные id CPU-веток не должны утечь на CUDA — оффлоад больших
    // батчей начинается с 32, так что ≤16 безопасно
    int32_t max_batch = 8;
